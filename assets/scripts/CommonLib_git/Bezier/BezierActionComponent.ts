    ...
    private updateRotation(currentPosition) {
        if(this.lastPosition)
        {
            if(this._lockVertical && this._lockHorizontal) {
                ...
                this.node.scaleX = (this.lastPosition.x <= currentPosition.x) ? Math.abs(this.node.scaleX) : -Math.abs(this.node.scaleX);
            }
            else {
                this.node.angle = -this.getRotation(currentPosition);
            }
        }
